    ...
    BEGIN TRY
		...
		SELECT 
			...
			CASE UPPER(TRIM(prd_line))
				...
				WHEN 'T' THEN 'Touring'
				ELSE 'Unknown'
			END AS prd_line, -- Map product line codes to descriptive and more friendly values
			CAST(prd_start_dt AS DATE) AS prd_start_dt,
			CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt) - 1 AS DATE) AS prd_end_dt -- Calculates end date as one day before the next start date
